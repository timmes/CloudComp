...
import {
  ...
  getUserWithPoints, getUserMonthPoints, getUserTotalPoints,
...
export function updateTeamPoints(teamId) {
  ...
  upsertTeam({ ...team, currentMonthPoints: monthPts, totalPoints: totalPts });
}
...
export function refreshTeamsTable() {
  ...
  if (teams.length === 0) {
    tbody.innerHTML = `<tr><td colspan="6" class="px-4 py-8 text-center text-gray-500">No teams created yet. Click "Create Team" to get started.</td></tr>`;
    return;
  }
...
export function updateTeamSortButtons() {
  ['sortMembers','sortTeamCurrentMonth','sortTeamTotal'].forEach(id => { const b = document.getElementById(id); if (b) b.className = 'px-3 py-1 text-xs rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors'; });
  ...
  const ab = document.getElementById(af); if (ab) ab.className = 'px-3 py-1 text-xs rounded-lg bg-blue-100 text-blue-800 border border-blue-300';
